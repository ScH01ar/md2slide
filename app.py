...
def generate_slides_with_gemini(md: str) -> str:
    ...
    prompt = (
        ...
        '2. 内容页优先使用 `layout: two-cols`、`layout: image-right`、`layout: center`、`layout: default`。\n'
        '3. 图片样式建议：`class="rounded-xl shadow-lg h-60 object-cover"`；如果尺寸不合适，可用 `h-72` 或 `w-full`。\n'
        '4. 若使用图标，优先 `<carbon-... />`；不确定时不要使用未知图标名称。\n'
        '5. 关键字可用简单的强调（如 `<span class="text-red-500">重点</span>`），避免过度装饰。\n\n'
        '代码与表格：\n'
